...
| **K06 („żeby nie było tego słowa w nawiasie") nie ma odpowiednika w danych.** W słowniku miast są `01. Orzesze` … `33. Tarnowskie Góry` — **żadnej wartości z nawiasem**. Nawiasy są tylko w słowniku trenerów (`11. Białas (Pszczyna)`) i w statusach | zapytanie do `slowniki` na kopii `prod` | **Nie wiemy, co poprawiać.** Pozycja idzie do pytań, ze zrzutem ekranu. Podejrzenie: Kasi chodzi o **prefiks `08. `**, a nie o nawias |
